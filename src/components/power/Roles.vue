<template>
  <div>
      ...
      <el-table :data="rolesList" border stripe>
        ...
        <el-table-column label="操作" width="270px">
          <template slot-scope="scope">
            ...
            <!-- 修改按钮 -->
            <el-button type="primary" class="el-icon-edit" size="mini" @click="showEditRoleDialog(scope.row.id)">编辑
            </el-button>

            <!-- 删除按钮 -->
            <el-button type="danger" class="el-icon-delete" size="mini" @click="removedRoleById(scope.row.id)">删除
            </el-button>
    ...
    <!-- 添加角色的对话框 -->
    <el-dialog title="添加新角色" :visible.sync="addDialogVisible" width="50%" @close="addRoleDialogClosed">
    ...
    <!-- 修改角色的对话框 -->
    <el-dialog title="修改角色信息" :visible.sync="editDialogVisible" width="50%" @close="editRoleDialogClosed">
  ...
  methods: {
    ...
    addRole () {
      this.$refs.addRoleFormRef.validate(async valid => {
        // console.log(valid)
        if (!valid) return this.$message.error('角色信息校验失败')
        // 可以发起添加角色的网络请求
    ...
    // 删除角色
    async removedRoleById (id) {
      // console.log(id)
      const { data: tipRes } = await this.$http.get('roles/' + id)
      // 弹框询问角色是否删除
      const confirmResult = await this.$confirm(`此操作将永久删除角色${tipRes.data.roleName}, 是否继续?`, '警告', {
      ...
      if (confirmResult !== 'confirm') return this.$message.info('已取消角色删除行为')
    ...
    async removeRightById (role, rightId) {
      // 弹框询问是否删除
      const confirmResult = await this.$confirm('此操作将永久删除此权限, 是否继续?', '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).catch(err => err)

      if (confirmResult !== 'confirm') return this.$message.info('取消了权限删除行为!')
    ...
    async showSetRightDialog (role) {
      ...
      // 递归获取三级节点的 id
      this.getLeafKeys(role, this.rightDefaultKeys)
      this.setRightDialogVisible = true
    },

    // 通过递归形式获取角色下所有权限 ID 并保存到 rightDefaultKeys
