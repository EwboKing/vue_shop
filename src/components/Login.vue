    ...
    <div class="login_box">
      ...
      <el-form ref="loginFormRef" :model="loginForm" :rules="loginFormRules" label-width="0px" class="login_form">
        ...
        <el-form-item prop="username">
          <el-input v-model="loginForm.username" placeholder="输入你的用户名" prefix-icon="iconfont icon-user-fill-copy">
          </el-input>
        </el-form-item>

        <!-- 密码 -->
        <el-form-item prop="password">
          <el-input v-model="loginForm.password" placeholder="输入你的密码" type="password"
            prefix-icon="iconfont icon-lock2-copy"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    login () {
      // axios.post('http://127.0.0.1:8888/api/private/v1/login', this.loginForm).then(
      //   response => {
      //     // console.log(response.data)
      //     if (response.data.meta.status !== 200) return this.$message.error('登录失败')
      //     this.$message.success('登录成功')
      //     // 1.将登录成功之后的 tolen，保存到客户端的 sessionStorage中
